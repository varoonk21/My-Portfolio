...
export default function Experience() {
  ...
  const lineHeight = useTransform(scrollYProgress, [0.15, 0.9], ["0%", "100%"]);
            ...
            {experiences.map((exp, index) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Card Container */}
                  <div
                    className={`w-full md:w-1/2 pl-14 sm:pl-16 md:pl-0 pt-0 ${isLeft ? "md:pr-10 lg:pr-14" : "md:pl-10 lg:pl-14"}`}
                  >
                    <div className="bg-white border border-neutral-200 rounded-2xl p-5 sm:p-6 lg:p-7 shadow-sm hover:shadow-lg transition-all duration-300 relative group">
                      <div
                        className={`hidden md:block absolute top-10 w-10 lg:w-14 h-0.5 bg-neutral-200 group-hover:bg-neutral-300 transition-colors duration-300 z-0 ${isLeft ? "right-0 translate-x-full" : "left-0 -translate-x-full"}`}
                      />

                      {/* Content */}
                      <div className="relative z-10 flex flex-row xl:items-start justify-between gap-3 mb-2">
                        <h3 className="text-xl sm:text-2xl font-bold text-neutral-900 leading-tight">{exp.company}</h3>
                        {exp.isCurrentRole && (
